...
void set_mac_address(char *data, uint8_t dlc)
{
	CANTxMsg_t  Msg;
...
void create_enable_wbms(char *data, uint8_t dlc)
{
...
int  main ( void)
{
	...
	while ( 1)
	{
		...
		if ( CAN_UserRead ( CAN_BUS1, &RxMsg) == CAN_ERR_OK)
		{
			...
			}else if( RxMsg.id == 0x200 ){
				if (toggle_first == 0){
					toggle_first = 1;
					memcpy(&can_data.data[0], &RxMsg.data8[0], RxMsg.dlc);			//64개
				}
				//SER_Write ( SER_PORT1, &RxMsg.data8[0], RxMsg.dlc);
			}else if( RxMsg.id == 0x201 ){
				if (toggle_first == 1){
					toggle_first = 2;
					memcpy(&can_data.data[64], &RxMsg.data8[0], RxMsg.dlc);			//16개
				}else{
					toggle_first = 0;
				}
				//SER_Write ( SER_PORT1, &RxMsg.data8[0], RxMsg.dlc);
			}else if( RxMsg.id == 0x202 ){
				if (toggle_first == 2){
					toggle_first = 3;
					memcpy(&can_data.data[80], &RxMsg.data8[0], RxMsg.dlc);			//64개
				}else{
					toggle_first = 0;
				}
				//SER_Write ( SER_PORT1, &RxMsg.data8[0], RxMsg.dlc);
			}else if( RxMsg.id == 0x203){
				if (toggle_first == 3){
					toggle_first = 0;
					memcpy(&can_data.data[144], &RxMsg.data8[0], RxMsg.dlc);			//16개
					en_queue();
				}else{
					toggle_first = 0;
				}
				//SER_Write ( SER_PORT1, "0x203", 5);
				//SER_Write ( SER_PORT1, &RxMsg.data8[0], RxMsg.dlc);
			...
			//SER_Write ( SER_PORT1, &RxMsg.id, 4);
			//if( RxMsg.id == 0x403  &&  (RxMsg.msgtype == CAN_MSGTYPE_EXTENDED || RxMsg.msgtype == CAN_MSGTYPE_STANDARD || RxMsg.msgtype == CAN_MSGTYPE_FDF) )
			//SER_Write ( SER_PORT1, &RxMsg.id, 2);
			// else
			// {
		...
		if (uart_status == idle_status){
			...
			if (serstat == SER_ERR_OK && ser_read > 0 ){
				if (temp == ETX){
					if (uart_data.cmd == 0xC6){
						uart_data.uart_read = 0;
						uart_status = parsing_status;
					}else{
		...
		}else if(uart_status == parsing_status){
			if (uart_data.cmd == 0xC6){
				status = de_queue();
				...
				if (status == NULL){
					...
					uart_data.temp = 0;
					uart_status = sending_status;
				}
			}
		}else if(uart_status == sending_status){
			if (uart_data.temp < 16){
				SER_Write ( SER_PORT1, &status[10*uart_data.temp], 10);
				uart_status = sending_status;
				uart_data.temp++;
				SER_Write ( SER_PORT1, &uart_data.temp, 1);
			}else{
				user_tx[4] = ETX;
